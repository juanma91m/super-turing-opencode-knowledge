...
if [[ "$#" -ne 2 || "$1" != "--collection" ]]; then
  printf 'Uso: knowledge_rebuild_collection.sh --collection <nombre>\n' >&2
  exit 2
fi
...
exec flock -x "$lock_file" "$python_bin" "$script_py" delete-collection --collection "$2" --confirm
