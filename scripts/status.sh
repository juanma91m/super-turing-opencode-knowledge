...
printf 'engram_session_context_plugin_present=%s\n' "$([[ -f "$TARGET_DIR/plugins/engram-session-context.ts" ]] && printf yes || printf no)"
printf 'knowledge_reader_present=%s\n' "$([[ -f "$TARGET_DIR/agents/knowledge-reader.md" ]] && printf yes || printf no)"
printf 'plan_autonomy_overlay_present=%s\n' "$([[ -f "$TARGET_DIR/agents/plan.md" ]] && grep -q 'KNOWLEDGE_AUTONOMY_START' "$TARGET_DIR/agents/plan.md" 2>/dev/null && printf yes || printf no)"
printf 'build_autonomy_overlay_present=%s\n' "$([[ -f "$TARGET_DIR/agents/build.md" ]] && grep -q 'KNOWLEDGE_AUTONOMY_START' "$TARGET_DIR/agents/build.md" 2>/dev/null && printf yes || printf no)"
